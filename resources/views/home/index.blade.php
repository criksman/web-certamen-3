      ...
      @foreach(Auth::user()->imagenes as $imagen)
        <div class="col p-5 d-flex flex-column justify-content-center align-items-center">
            ...
            @if($imagen->baneada == 1)
              ...
              <button type="button" class="btn btn-danger mb-3 text-white" data-bs-toggle="modal" data-bs-target="#exampleModal{{$imagen->id}}">
                Motivo de baneo
              </button>
            ...
            <!-- Button trigger modal -->
            <form method="POST" class="d-flex align-items-center" action="{{ route('artista.destroyImagen', $imagen->id) }}">
            <a class="btn btn-secondary me-2" href="{{route('artista.edit_imagen', $imagen->id)}}">Editar</a>
              @method('delete')
              @csrf
              ...
              <div class="modal fade" id="exampleModal2{{$imagen->id}}" tabindex="-1" aria-labelledby="exampleModalLabel2{{$imagen->id}}" aria-hidden="true">
                <div class="modal-dialog">
                  <div class="modal-content">
                    <div class="modal-header">
                      <h1 class="modal-title fs-5 text-black" id="exampleModalLabel2{{$imagen->id}}">Eliminar imagen</h1>
                      <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
