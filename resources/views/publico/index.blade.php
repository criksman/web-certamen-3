...
<div class="row text-white">
    <div class="col pt-3">
        <h1 class="d-flex flex-column justify-content-center align-items-center">Publicaciones</h1>
    </div>
</div>
        ...
        <div class="row">
            @if($imagenesFiltradas != null)
                @foreach ($imagenesFiltradas as $imagen)
                    <div class="col p-5 d-flex flex-column justify-content-center align-items-center @if($imagen->baneada == 1 && Gate::denies('administracion-mostrar')) d-none @endif">
                        ...
                        @if(Gate::allows('administracion-mostrar'))
                            @if($imagen->baneada == 0)
                                <form method="POST" action="{{ route('admin.banImagen', $imagen->id) }}">
                                    @method('put')
                                    @csrf
                                    <button type="button" class="btn btn-warning text-white" data-bs-toggle="modal" data-bs-target="#exampleModal{{$imagen->id}}" data-bs-whatever="@mdo">Banear publicación</button>
